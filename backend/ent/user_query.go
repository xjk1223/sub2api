...
type UserQuery struct {
	...
	withCheckinLogs           *CheckinLogQuery
...
func (_q *UserQuery) QueryCheckinLogs() *CheckinLogQuery {
	query := (&CheckinLogClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) WithCheckinLogs(opts ...func(*CheckinLogQuery)) *UserQuery {
	query := (&CheckinLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withCheckinLogs; query != nil {
		if err := _q.loadCheckinLogs(ctx, query, nodes,
			func(n *User) { n.Edges.CheckinLogs = []*CheckinLog{} },
			func(n *User, e *CheckinLog) { n.Edges.CheckinLogs = append(n.Edges.CheckinLogs, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadCheckinLogs(ctx context.Context, query *CheckinLogQuery, nodes []*User, init func(*User), assign func(*User, *CheckinLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(checkinlog.FieldUserID)
	}
	query.Where(predicate.CheckinLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.CheckinLogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
