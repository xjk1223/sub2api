...
// The CheckinLogFunc type is an adapter to allow the use of ordinary
// function as CheckinLog mutator.
type CheckinLogFunc func(context.Context, *ent.CheckinLogMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f CheckinLogFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.CheckinLogMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.CheckinLogMutation", m)
}
