...
// The CheckinLogFunc type is an adapter to allow the use of ordinary function as a Querier.
type CheckinLogFunc func(context.Context, *ent.CheckinLogQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f CheckinLogFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.CheckinLogQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.CheckinLogQuery", q)
}

// The TraverseCheckinLog type is an adapter to allow the use of ordinary function as Traverser.
type TraverseCheckinLog func(context.Context, *ent.CheckinLogQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseCheckinLog) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseCheckinLog) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.CheckinLogQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.CheckinLogQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.CheckinLogQuery:
		return &query[*ent.CheckinLogQuery, predicate.CheckinLog, checkinlog.OrderOption]{typ: ent.TypeCheckinLog, tq: q}, nil
